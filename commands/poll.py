...
class Poll(Command):
    ...
    async def execute(self):
        print('User %s executed poll %s' % (str(self.author), self.command.get('used', 'tdb!poll')))
        if not await self.run_test():
            return

        if not self.params or len(self.params) == 0:
            return await self.message.channel.send('Syntax: %s {title} {question} {optional answers(max 5)}' % (self.command.get('used', 'tdb!poll'), ))
