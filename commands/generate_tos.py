...
class Generatetos(Command): 
    ...
    async def execute(self):
        ...
        if None is not self.params and 'embi' in self.params:
            await self.message.channel.send('GTFO, i dont support embi in any way anymore!')
        ...
        if None is not self.params and 'kitty' in self.params:
            ...
            embed = discord.Embed(title='%s - Server ToS' % self.message.guild.name, description='', color=config.embed_color, colour=config.embed_color)
            embed.add_field(name='Rules', value=rules, inline=False)
        ...
        message = await self.message.channel.send(self.message.channel, embed=embed)
        return message
