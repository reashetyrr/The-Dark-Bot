...
class Rolemenu:
    ...
    def get_by_id(cls, rolemenu_id: int):
        result = DB().fetch_one(f'SELECT {get_fields()} FROM `rolemenus` WHERE id=?', [rolemenu_id])
        return cls(*result) if result else None

    @classmethod
    def get_by_message_id(cls, message_id: int):
        result = DB().fetch_one(f'SELECT {get_fields()} FROM rolemenus WHERE message_id=?', [message_id])
        return cls(*result) if result else None
    ...
    def get_all_by_guild(cls, guild_id: int):
        results = DB().fetch_all(f'SELECT {get_fields()} FROM rolemenus WHERE guild_id=?', [guild_id])
        rolemenus = [cls(*result) for result in results] if results else None
        return rolemenus

    @classmethod
    def get_by_name_and_id(cls, name: str, guild_id: int):
        result = DB().fetch_one(f'SELECT {get_fields()} FROM rolemenus WHERE name=? AND guild_id=?', [name, guild_id])
        return cls(*result) if result else None

    @classmethod
    def get_by_id_and_guild(cls, menu_id: int, guild_id: int):
        result = DB().fetch_one(f'SELECT {get_fields()} FROM rolemenus WHERE id=? AND guild_id=?', [menu_id, guild_id])
        return cls(*result) if result else None

    def save(self):
        query = f'INSERT OR REPLACE INTO rolemenus({get_fields()}) VALUES(?,?,?,?)'
        values = [
            self.guild_id,
            self.name,
            self.messsage_id,
    ...
    def delete(self):
        query = f'DELETE FROM rolemenus WHERE id=?'
        values = [self.id]
